...
OutIter lzw_decompress(Iter first, Iter last, OutIter out)
{
    ...
    boost::container::small_vector<unsigned char, 256> table_entry;
...
struct lzw_to_break_prop_iter
{
    ...
    using buffer_t = boost::container::small_vector<unsigned char, 256>;
    ...
    lzw_to_break_prop_iter & operator=(unsigned char c)
    {
        ...
        auto it = buf_->begin();
        for (auto end_ = buf_->end() - buf_->size() % element_bytes;
             it != end_;
             it += element_bytes) {
...
boost::container::small_vector<unsigned char, 256> buf;
std::copy(
    compressed.begin(),
    compressed.end(),
